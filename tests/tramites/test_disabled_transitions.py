"""
Tests for BACKOFFICE_DISABLED_TRANSITIONS filtering.

Validates that _get_disabled_transitions() and Tramite.available_actions()
correctly filter workflow transitions based on settings.BACKOFFICE_DISABLED_TRANSITIONS.
...
def test_no_disabled_returns_empty_set():
    """Without BACKOFFICE_DISABLED_TRANSITIONS, the set is empty."""
    with override_settings(BACKOFFICE_DISABLED_TRANSITIONS=[]):
        assert _get_disabled_transitions() == set()


@pytest.mark.django_db
def test_disabled_transitions_are_returned_as_int_set():
    """BACKOFFICE_DISABLED_TRANSITIONS values are returned as a set of ints."""
    with override_settings(BACKOFFICE_DISABLED_TRANSITIONS=[205, 203]):
