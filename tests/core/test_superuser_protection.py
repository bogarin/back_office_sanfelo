...
class TestSuperuserChangeProtection:
    """Non-superusers cannot change superusers."""

    def test_administrador_cannot_change_superuser(self, admin_user, superuser):
    ...
    def test_superuser_can_change_superuser(self, superuser):
        from django.contrib.auth import get_user_model
        ...
        assert model_admin.has_change_permission(request, obj=other_superuser) is True


class TestSuperuserDeleteProtection:
    """Non-superusers cannot delete superusers."""

    def test_administrador_cannot_delete_superuser(self, admin_user, superuser):
        model_admin = _get_model_admin()
        request = _build_request(admin_user)
        assert model_admin.has_delete_permission(request, obj=superuser) is False

    def test_superuser_can_delete_superuser(self, superuser):
        from django.contrib.auth import get_user_model

        User = get_user_model()
        other_superuser = User.objects.create_superuser(
            username='other_super2',
            email='other2@example.com',
            password='pass',
        )
        model_admin = _get_model_admin()
        request = _build_request(superuser)
        assert model_admin.has_delete_permission(request, obj=other_superuser) is True


class TestSuperuserReadonlyFields:
    """All fields are readonly when non-superuser views a superuser."""

    def test_administrador_sees_readonly_fields_for_superuser(self, admin_user, superuser):
        model_admin = _get_model_admin()
        request = _build_request(admin_user)
    ...
    def test_administrador_sees_normal_readonly_for_regular(self, admin_user, db):
        from django.contrib.auth import get_user_model
...
class TestSuperuserAccionesColumn:
    """Password link is hidden for superusers when viewer is not superuser."""

    def test_administrador_no_password_link_for_superuser(self, admin_user, superuser):
        model_admin = _get_model_admin()
        request = _build_request(admin_user)
        model_admin._request = request  # set by changelist_view
        result = model_admin.acciones(superuser)
        assert 'Cambiar password' not in result
        assert result == '—'

    def test_superuser_sees_password_link_for_superuser(self, superuser):
        from django.contrib.auth import get_user_model

        User = get_user_model()
        other_superuser = User.objects.create_superuser(
            username='other_super3',
        ...
        result = model_admin.acciones(other_superuser)
        assert 'Cambiar password' in result


class TestSuperuserBulkActionProtection:
    """Bulk actions exclude superusers for non-superuser editors."""

    def test_marcar_como_activo_excludes_superusers(self, admin_user, superuser, db):
        from django.contrib.auth import get_user_model
    ...
    def test_marcar_como_inactivo_excludes_superusers(self, admin_user, superuser, db):
        from django.contrib.auth import get_user_model
...
# ---------------------------------------------------------------------------
# is_staff lifecycle tests
# ---------------------------------------------------------------------------


class TestIsStaffLifecycle:
    """is_staff is set to True for any valid role, False when role removed."""
    ...
    def test_new_user_with_role_gets_is_staff(self, superuser, db, role):
        from django.contrib.auth import get_user_model
    ...
    def test_new_user_is_active(self, superuser, db):
        from django.contrib.auth import get_user_model
    ...
    def test_role_removed_sets_is_staff_false(self, superuser, admin_user, db):
        from django.contrib.auth import get_user_model
    ...
    def test_soft_delete_preserves_group_and_is_staff(self, admin_user, admin_group, db):
        from django.contrib.auth import get_user_model
    ...
    def test_reactivate_user_preserves_group_and_is_staff(self, admin_user, admin_group, db):
        from django.contrib.auth import get_user_model
